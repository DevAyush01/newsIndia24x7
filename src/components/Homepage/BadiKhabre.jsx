  ...
  return (
    <section className="container max-w-7xl mx-auto py-5 px-4">
      ...
      <div className="grid grid-cols-12 gap-5">
        ...
        {/* ========================================================= */}
        {/* CENTER - Image News (3 columns grid) */}
        {/* ========================================================= */}
        <div className="col-span-12 lg:col-span-5">
          <div className="grid grid-cols-3 gap-4 ">
            {coverStoryBig.slice(0, 3).map((post) => (
              <Link key={post.id} href={`/post/${post.slug}`}>
                <article className="group cursor-pointer bg-[#F5F5F5] border border-[#d1d1d1] h-80">
                  ...
                  <h3 className="mt-3  text-[15px] p-2 font-bold text-gray-900 group-hover:text-red-600 transition-colors line-clamp-6">
                    {post.title}
                  </h3>
                </article>
              </Link>
              ...
              {newsLatest.slice(0, 3).map((post) => (
                <Link key={post.id} href={`/post/${post.slug}`}>
                  <div className="flex gap-4 items-start group cursor-pointer  p-1 py-2 ">
                    <div className="relative w-30 h-16 shrink-0 overflow-hidden bg-gray-100 ">
                      {post.featuredImage?.node?.sourceUrl ? (
                        <Image
                          src={post.featuredImage.node.sourceUrl}
                          alt={post.title}
                    ...
                    <h3 className="text-sm font-bold leading-5 text-gray-900 group-hover:text-red-600 transition-colors line-clamp-3">
                      {post.title}
                    </h3>
                  </div>
                </Link>
